#!/bin/bash
# Deploy dchat binary to 3 Azure servers via WSL (with service stop/start)

set -e

echo "=== dchat 3-Server Deployment Script (WSL v2) ==="
echo "Date: $(date)"
...
# Check SSH keys
echo -e "\n${YELLOW}[2/7] Checking SSH keys...${NC}"
...
echo -e "\n${YELLOW}[3/7] Stopping existing services...${NC}"
for server_info in "${SERVERS[@]}"; do
    IFS=':' read -r name ip user key_path domain <<< "$server_info"
    
    echo -e "${CYAN}   Stopping service on $name...${NC}"
    ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -i "$key_path" "$user@$ip" \
        "sudo systemctl stop dchat 2>/dev/null || true"
    sleep 2
    echo -e "${GREEN}   ✅ Service stopped${NC}"
done

# Upload binary to all servers
echo -e "\n${YELLOW}[4/7] Uploading binary to servers...${NC}"
for server_info in "${SERVERS[@]}"; do
    IFS=':' read -r name ip user key_path domain <<< "$server_info"
...
echo -e "\n${YELLOW}[5/7] Generating configurations...${NC}"
...
echo -e "\n${YELLOW}[6/7] Setting up systemd services...${NC}"
...
${CYAN}To check connectivity:${NC}
  wsl bash /mnt/c/Users/USER/dchat/check-3-server-connectivity.sh
"
