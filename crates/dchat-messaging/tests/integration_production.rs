...
mod tests {
    ...
    fn test_production_staking_verifier_wired() {
        ...
        assert!(
            staking_verifier.is_ok(),
            "ChainStakingVerifier should be constructible"
        );
        ...
        assert_eq!(
            channel_manager
                .get_members(&dchat_core::types::ChannelId::new())
                .len(),
            0
        );
    }
    ...
    fn test_production_config_values() {
        ...
        assert!(
            config.per_user_limit < 1000,
            "Per-user limit should prevent spam"
        );
        assert!(
            config.max_queue_size < 1000000,
            "Queue should have bounded memory"
        );
    }
}
