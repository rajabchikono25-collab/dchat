...
impl SignatureFreshnessCheck {
    ...
    pub fn check_freshness(
        validator_id: String,
        last_signature_time: SystemTime,
    ) -> Self {
...
impl EnhancedHealthCheck {
    ...
    pub fn to_validator_health(&self) -> ValidatorHealth {
        ValidatorHealth {
            ...
            block_height: self.block_height_check
                .as_ref()
    ...
    fn calculate_uptime(&self) -> f64 {
        if self.consecutive_failures == 0 {
            ...
            let failure_rate = (self.consecutive_failures as f64) / (MAX_CONSECUTIVE_FAILURES as f64);
...
impl EnhancedHealthChecker {
    ...
    pub async fn check_validator(
        ...
        // Check signature freshness
        let signature_freshness_check = last_signature_time.map(|time| {
            SignatureFreshnessCheck::check_freshness(validator_id.clone(), time)
        });
        
        // Update failure count
        let consecutive_failures = if probe.success {
            // Reset on success
            self.failure_counts.write().unwrap().insert(validator_id.clone(), 0);
            0
        } else {
        ...
        history.entry(validator_id.clone()).or_insert_with(Vec::new).push(check.clone());
    ...
    async fn execute_probe(&self, probe: &mut HealthProbe) -> Result<(), HealthCheckError> {
        ...
        if self.test_mode && probe.endpoint.contains("invalid") {
            return Err(HealthCheckError::Unreachable("Invalid endpoint".to_string()));
        }
...
mod tests {
    ...
    fn test_block_height_sync() {
        let check = BlockHeightCheck::check_sync(
            "validator1".to_string(),
            100,
            105,
        );
    ...
    fn test_signature_freshness() {
        ...
        let check = SignatureFreshnessCheck::check_freshness(
            "validator1".to_string(),
            recent,
        );
    ...
    async fn test_enhanced_health_check() {
        ...
        let result = checker.check_validator(
            "validator1".to_string(),
            verifying_key,
            "http://localhost:9090".to_string(),
            98, // Slight lag
