...
use dchat_network::nat_traversal::{NatConfig, NatTraversalManager, NatType, NatStrategy};
...
async fn test_nat_detection_with_public_stun() {
    ...
    match timeout(Duration::from_secs(15), manager.detect_nat_type()).await {
        Ok(Ok(nat_type)) => {
            ...
            match nat_type {
                NatType::None | NatType::Open | NatType::FullCone | 
...
fn test_stun_binding_request_format() {
    ...
    let request = manager.build_stun_binding_request().unwrap();
...
fn test_stun_response_parsing() {
    ...
    response.extend_from_slice(&[0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C]);
...
fn test_strategy_recommendation() {
    ...
    manager.detected_nat_type = Some(NatType::RestrictedCone);
    assert_eq!(manager.get_recommended_strategy(), NatStrategy::UPnP);
    
    // Test strategy for PortRestrictedCone NAT
    manager.detected_nat_type = Some(NatType::PortRestrictedCone);
    assert_eq!(manager.get_recommended_strategy(), NatStrategy::HolePunching);
    ...
    manager.detected_nat_type = Some(NatType::Unknown);
    assert_eq!(manager.get_recommended_strategy(), NatStrategy::TURN);
}
...
fn test_turn_allocate_request_format() {
    ...
    let request = manager.build_turn_allocate_request(username, credential).unwrap();
    ...
    assert!(has_transport, "TURN request should have REQUESTED-TRANSPORT attribute");
    ...
    assert!(has_integrity, "TURN request should have MESSAGE-INTEGRITY attribute");
}
...
async fn test_hole_punching_timeout() {
    ...
    match timeout(
        ...
        Ok(Ok(success)) => {
            ...
            assert!(!success, "Hole punching should fail with non-routable addresses");
        }
        Ok(Err(e)) => {
...
fn test_nat_type_classification() {
    ...
    for nat_type in nat_types {
        manager.detected_nat_type = Some(nat_type);
        let strategy = manager.get_recommended_strategy();
        
        // Verify every NAT type maps to a valid strategy
        match strategy {
            NatStrategy::Direct | NatStrategy::UPnP | 
            NatStrategy::HolePunching | NatStrategy::TURN | NatStrategy::Turn => {
