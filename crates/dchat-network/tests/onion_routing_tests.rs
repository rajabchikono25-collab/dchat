...
#[test]
fn test_single_hop_encryption_decryption() {
    // Create a single-hop circuit
    let mut router = OnionRouter::new();
    let relay_peer = PeerId::random();
    
    router.create_circuit("test-circuit".to_string(), vec![relay_peer]).unwrap();
...
fn test_three_hop_encryption() {
    ...
    router.create_circuit("test-circuit-3hop".to_string(), relays.clone()).unwrap();
    ...
    assert!(onion.len() >= min_expected_size, 
    ...
    println!("Overhead per layer: ~{} bytes", (onion.len() - plaintext.len()) / relays.len());
}
...
fn test_circuit_closure() {
    ...
    let relays = vec![PeerId::random(), PeerId::random()];
    router.create_circuit("circuit-close-test".to_string(), relays).unwrap();
...
fn test_encryption_produces_different_ciphertexts() {
    ...
    let mut router = OnionRouter::new();
    let relays = vec![PeerId::random(), PeerId::random()];
    router.create_circuit("test-nonce-uniqueness".to_string(), relays.clone()).unwrap();
    ...
    assert_ne!(onion1, onion2, "Nonce reuse detected! This is a critical security vulnerability.");
}
...
fn test_long_message_encryption() {
    ...
    let mut router = OnionRouter::new();
    let relays = vec![PeerId::random(), PeerId::random(), PeerId::random()];
    router.create_circuit("large-msg-circuit".to_string(), relays).unwrap();
...
fn test_empty_message_encryption() {
    ...
    let mut router = OnionRouter::new();
    let relays = vec![PeerId::random(), PeerId::random()];
    router.create_circuit("empty-msg-circuit".to_string(), relays).unwrap();
...
fn test_maximum_circuit_length() {
    ...
    assert!(onion.len() < max_expected_size * 2, "Onion packet too large");
}
...
mod security_tests {
    ...
    fn test_nonce_uniqueness_across_layers() {
        ...
        let mut router = OnionRouter::new();
        let relays = vec![PeerId::random(), PeerId::random(), PeerId::random()];
        router.create_circuit("nonce-uniqueness-test".to_string(), relays).unwrap();
        ...
        println!("Extracted {} nonce positions from onion packet", nonces.len());
    }
    ...
    fn test_replay_attack_resistance() {
        ...
        let mut router = OnionRouter::new();
        let relays = vec![PeerId::random(), PeerId::random()];
        router.create_circuit("replay-test".to_string(), relays).unwrap();
...
mod performance_tests {
    ...
    fn bench_encryption_performance() {
        let mut router = OnionRouter::new();
        let relays = vec![PeerId::random(), PeerId::random(), PeerId::random()];
        router.create_circuit("perf-test".to_string(), relays).unwrap();
        ...
        assert!(avg_time.as_millis() < 10, "Encryption too slow: {:?}", avg_time);
    }
}
