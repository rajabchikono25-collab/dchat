...
impl NatTraversalManager {
    ...
    pub async fn detect_nat_type(&mut self) -> Result<NatType> {
        ...
        let secondary_stun: SocketAddr = self.config.stun_servers
            .get(1)
        ...
        let response1 = timeout(Duration::from_secs(5), local_socket.recv_from(&mut buf)).await
        ...
        let nat_type = if mapped_addr1 == local_addr {
            ...
            let response2 = timeout(Duration::from_secs(5), local_socket.recv_from(&mut buf)).await
    ...
    fn parse_stun_response(&self, data: &[u8]) -> Result<SocketAddr> {
        ...
        Err(Error::network("XOR-MAPPED-ADDRESS not found in STUN response"))
    }
    ...
    pub async fn setup_upnp(&mut self, internal_port: u16) -> Result<SocketAddr> {
        ...
        let external_port = self.request_upnp_port_mapping(
            &gateway_addr,
            internal_port,
    ...
    async fn get_upnp_external_ip(&self, _gateway: &SocketAddr) -> Result<std::net::IpAddr> {
        ...
        Err(Error::network("External IP detection not fully implemented"))
    }
    ...
    pub async fn setup_turn(&mut self, username: String, credential: String) -> Result<SocketAddr> {
        ...
        local_socket.send_to(&allocate_request, turn_server).await
    ...
    fn build_turn_allocate_request(&self, username: &str, credential: &str) -> Result<Vec<u8>> {
        ...
        let mut mac = HmacSha1::new_from_slice(&key)
            .map_err(|_| Error::network("Failed to create HMAC"))?;
        mac.update(&request);
    ...
    fn parse_turn_allocate_response(&self, data: &[u8]) -> Result<SocketAddr> {
        ...
        Err(Error::network("XOR-RELAYED-ADDRESS not found in TURN response"))
    }
    
    /// Setup TURN relay connection (legacy interface)
    pub async fn _setup_turn_legacy(&mut self, username: String, credential: String) -> Result<SocketAddr> {
        if self.config.turn_servers.is_empty() {
            return Err(Error::network("No TURN servers configured"));
        }
