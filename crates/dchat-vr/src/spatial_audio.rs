...
impl HrtfProcessor {
    ...
    pub fn calculate_itd(&self, azimuth: f32) -> f32 {
        ...
        let itd_seconds = (self.head_radius / self.speed_of_sound) * (azimuth.sin() + azimuth);
...
impl RoomAcoustics {
    pub fn new(room_size: Vector3, wall_absorption: f32) -> Self {
        ...
        let surface_area = 2.0
            * (room_size.x * room_size.y + room_size.y * room_size.z + room_size.z * room_size.x);
    ...
    pub fn calculate_reverb_mix(&self, distance: f32) -> f32 {
        ...
        let critical_distance = 0.057
            * (self.room_size.x * self.room_size.y * self.room_size.z).sqrt()
...
impl Default for RoomAcoustics {
    fn default() -> Self {
        ...
        Self::new(Vector3::new(10.0, 3.0, 8.0), 0.3)
    }
}
...
impl SpatialAudioManager {
    ...
    fn calculate_doppler_shift(
        ...
        let source_radial_velocity = source_velocity.x * direction.x
            + source_velocity.y * direction.y
            + source_velocity.z * direction.z;

        let listener_radial_velocity = self.listener.velocity.x * direction.x
    ...
    pub fn calculate_spatial_params(&self, user_id: &UserId) -> Option<SpatialAudioParams> {
        ...
        let right = Vector3 {
            x: self.listener.forward.y * self.listener.up.z
                - self.listener.forward.z * self.listener.up.y,
            y: self.listener.forward.z * self.listener.up.x
                - self.listener.forward.x * self.listener.up.z,
            z: self.listener.forward.x * self.listener.up.y
                - self.listener.forward.y * self.listener.up.x,
        };
    ...
    pub fn get_early_reflections(&self, user_id: &UserId) -> Option<Vec<EarlyReflection>> {
        ...
        Some(
            self.room_acoustics
                .calculate_early_reflections(&source.position, &self.listener.position),
        )
    }
...
mod tests {
    ...
    fn test_reverb_mix_distance() {
        let manager = SpatialAudioManager::new_with_room(Vector3::new(10.0, 3.0, 8.0), 0.3);
