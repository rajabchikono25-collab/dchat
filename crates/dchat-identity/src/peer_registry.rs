...
impl PeerRegistry {
    ...
    pub fn get_peer(&self, peer_id: &PeerId) -> Result<Option<AuthenticatedPeer>, RegistryError> {
        let peers = self.peers.read().map_err(|_| RegistryError::LockPoisoned)?;
    ...
    pub fn get_peers_by_role(
        &self,
        role: PeerRole,
    ) -> Result<Vec<AuthenticatedPeer>, RegistryError> {
        let peers = self.peers.read().map_err(|_| RegistryError::LockPoisoned)?;

        Ok(peers.values().filter(|p| p.role == role).cloned().collect())
    }
    ...
    pub fn prune_inactive(
        &self,
        inactive_threshold: std::time::Duration,
        ...
        peers.retain(|_, peer| now.duration_since(peer.last_active) < inactive_threshold);
