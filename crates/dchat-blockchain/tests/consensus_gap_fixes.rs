...
async fn test_dilithium3_signature_verification() {
    ...
    assert!(
        result.is_err(),
        "Wrong message should fail Dilithium3 verification"
    );
...
async fn test_hybrid_signature_verification() {
    ...
    let dil_result = hybrid_sig
        .dilithium3
        .verify(message, dilithium_keypair.public_key.as_bytes());
...
async fn test_vote_persistence_porw() {
    ...
    let persistence = VotePersistence::new(&database_url)
        .await
    ...
    // Store vote
    persistence
        .store_porw_vote(&vote)
        .await
        .expect("Failed to store PoRW vote");

    // Retrieve votes
    let votes = persistence
        .get_porw_votes("test_block_123")
        .await
    ...
    let has_voted = persistence
        .check_porw_double_vote("test_block_123", &keypair.verifying_key().to_bytes())
        .await
        .expect("Failed to check double-vote");
...
fn test_geoip_lookup() {
    ...
    let location = manager.lookup(ip).expect("Failed to lookup Google DNS IP");
...
fn test_geographic_quorum_verification() {
    ...
    println!(
        "  Max concentration: {:.2}%",
        quorum.max_continent_concentration * 100.0
    );
    println!("  Meets diversity: {}", quorum.meets_minimum_diversity);
    println!(
        "  Meets concentration limit: {}",
        quorum.meets_concentration_limit
    );
...
async fn test_oracle_network_registration() {
    ...
    assert!(
        result.is_ok(),
        "Should register oracle with sufficient stake"
    );
    ...
    assert!(
        result.is_err(),
        "Should reject oracle with insufficient stake"
    );
...
fn test_distance_calculation() {
    ...
    assert!(
        distance > 5500.0 && distance < 5600.0,
        "Distance should be ~5570km, got {}km",
        distance
    );
...
async fn test_transit_path_verification_with_dilithium3() {
    ...
    let path = TransitPath {
        relays: vec![relay1_ed.verifying_key(), relay2_ed.verifying_key()],
        dilithium_keys: vec![
    ...
    assert!(
        sol_result.is_ok(),
        "Should pass speed of light verification: {:?}",
        sol_result
    );
