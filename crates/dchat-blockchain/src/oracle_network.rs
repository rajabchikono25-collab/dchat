...
impl OracleNetwork {
    ...
    pub fn register_oracle(&self, pubkey: VerifyingKey, stake_amount: u64) -> Result<()> {
        ...
        tracing::info!("Oracle registered: {:?} with stake {}", pubkey, stake_amount);
    ...
    pub fn submit_prediction(&self, prediction: OraclePrediction) -> Result<()> {
        ...
        let mut oracles = self.oracles.write().unwrap();
        let oracle = oracles
            .get_mut(&prediction.oracle_pubkey)
            .ok_or_else(|| OracleError::OracleNotRegistered(format!("{:?}", prediction.oracle_pubkey)))?;
        
        if !oracle.is_active {
            return Err(OracleError::OracleSlashed(format!("{:?}", prediction.oracle_pubkey)));
        }
    ...
    pub fn get_consensus(&self, prediction_type: PredictionType) -> Result<OracleConsensus> {
        ...
        let type_predictions = predictions
            .get(&prediction_type)
            .ok_or_else(|| OracleError::OracleNotRegistered("No predictions available".to_string()))?;
        
        if type_predictions.is_empty() {
            return Err(OracleError::OracleNotRegistered("No predictions available".to_string()));
        }
        
        // Filter fresh predictions (last 5 minutes)
        let fresh_predictions: Vec<_> = type_predictions
            .iter()
            .filter(|p| p.is_fresh())
            .collect();
            ...
            if relative_error > 0.5 && pred.confidence > 0.7 {
                if let Some(oracle) = oracles.get_mut(&pred.oracle_pubkey) {
                    ...
                    if oracle.slashed_amount > oracle.stake_amount {
                        ...
                        tracing::warn!("Oracle {:?} deactivated due to excessive slashing", pred.oracle_pubkey);
                    }
                }
            } else {
