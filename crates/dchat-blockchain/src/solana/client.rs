...
use super::rpc::{SolanaRpcClient, SolanaRpcConfig, Commitment, RpcError, AccountInfoResponse};
...
use crate::wallet::solana_compat::{SolanaAddress, base58_decode};
...
impl SolanaClient {
    ...
    pub fn with_bridge(mut self, program_id: SolanaAddress, wdchat_mint: SolanaAddress) -> Result<Self> {
    ...
    pub async fn get_wdchat_balance(&self, owner: &SolanaAddress) -> Result<u64> {
        let mint = self.wdchat_mint.as_ref()
        ...
        match self.rpc.get_token_account_balance(&ata).await {
            ...
            Err(e) => Err(Error::network(format!("Failed to get token balance: {}", e))),
        }
    }

    /// Get account info
    pub async fn get_account(&self, address: &SolanaAddress) -> Result<Option<AccountInfoResponse>> {
        self.rpc.get_account_info(address).await
    ...
    pub async fn get_blockhash_bytes(&self) -> Result<[u8; 32]> {
        let response = self.rpc.get_latest_blockhash().await
    ...
    pub async fn transfer_wdchat(
        ...
        let mut builder = TransactionBuilder::new()
            .payer(from_owner.clone());
        
        // Create ATA if needed
        if !self.account_exists(&to_ata).await? {
            let create_ata_ix = SplToken::create_associated_token_account_instruction(
                from_owner,
                to_owner,
                mint,
            )?;
    ...
    pub async fn send_and_confirm(
        ...
        self.rpc.confirm_transaction(&signature, std::time::Duration::from_secs(60)).await
    ...
    pub async fn request_airdrop(&self, address: &SolanaAddress, amount_sol: f64) -> Result<String> {
        ...
        self.rpc.request_airdrop(address, lamports).await
    ...
    pub async fn get_rent_exemption(&self, data_len: usize) -> Result<u64> {
        self.rpc.get_minimum_balance_for_rent_exemption(data_len).await
    ...
    pub async fn simulate(&self, tx: &SolanaTransaction) -> Result<()> {
        let tx_base64 = tx.to_base64()?;
        self.rpc.simulate_transaction(&tx_base64).await
