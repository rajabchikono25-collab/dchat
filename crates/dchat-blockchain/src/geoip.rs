...
impl GeoIPManager {
    ...
    pub fn new<P: AsRef<Path>>(database_path: P) -> Result<Self> {
        let reader = Reader::open_readfile(database_path.as_ref())
            .map_err(|e| GeoIPError::DatabaseNotFound(format!("{:?}: {}", database_path.as_ref(), e)))?;
    ...
    pub fn lookup(&self, ip: IpAddr) -> Result<GeoLocation> {
        let city: geoip2::City = self.reader
            .lookup(ip)
    ...
    pub fn calculate_diversity_score(&self, ips: &[IpAddr]) -> f64 {
        ...
        let locations: Vec<GeoLocation> = ips
            .iter()
    ...
    pub fn verify_geographic_quorum(&self, ips: &[IpAddr]) -> Result<GeographicQuorum> {
        ...
        if locations.is_empty() {
            return Err(GeoIPError::AddressNotFound("No valid locations found".to_string()));
        }
        
        // Count nodes per continent
        let mut continent_counts = std::collections::HashMap::new();
        for loc in &locations {
            *continent_counts.entry(loc.continent_code.clone()).or_insert(0) += 1;
        }
