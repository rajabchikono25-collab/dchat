...
impl EpochSnapshot {
    ...
    pub fn finalize(&mut self) {
        ...
        let total_raw: u64 = self.relay_weights.values()
            .filter(|r| r.active)
            .map(|r| r.raw_score)
            .sum();
        ...
        self.total_stake_power = self.stake_entries.values()
    ...
    pub fn get_relay_weight(&self, relay_id: &[u8; 32]) -> Option<u64> {
        self.relay_weights.get(relay_id)
            .filter(|r| r.active)
            .map(|r| r.weight_bps)
    }
    ...
    pub fn get_stake_power(&self, staker_id: &[u8; 32]) -> Option<u64> {
        self.stake_entries.get(staker_id)
            .map(|s| s.effective_power)
    }
...
impl PoRWThresholdCalculator {
    ...
    pub fn add_vote(&mut self, relay_id: [u8; 32], vote: bool) -> Result<(), ThresholdError> {
        ...
        let weight = self.snapshot.get_relay_weight(&relay_id)
...
impl TSCThresholdCalculator {
    ...
    pub fn add_vote(&mut self, staker_id: [u8; 32], vote: bool) -> Result<(), ThresholdError> {
        ...
        let power = self.snapshot.get_stake_power(&staker_id)
...
impl EpochSnapshotManager {
    ...
    pub fn create_porw_calculator(&self, block: u64) -> Result<PoRWThresholdCalculator, ThresholdError> {
        let snapshot = self.get_threshold_snapshot(block)?;
        PoRWThresholdCalculator::new(snapshot)
    }
    
    /// Create TSC calculator for a block
    pub fn create_tsc_calculator(&self, block: u64) -> Result<TSCThresholdCalculator, ThresholdError> {
        let snapshot = self.get_threshold_snapshot(block)?;
