...
impl Frame {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, FramingError> {
        ...
        let header_bytes: &[u8; FRAME_HEADER_SIZE] = bytes[0..FRAME_HEADER_SIZE].try_into().unwrap();
...
impl CookieSecretManager {
    ...
    pub fn verify_cookie(&mut self, cookie: &Cookie, client_addr: &SocketAddr) -> Result<(), FramingError> {
        self.maybe_rotate();
...
impl ServerHandshake {
    ...
    pub fn process_init(
        ...
        *self.pending_count.entry(ip).or_insert(0) += 1;
        ...
        Ok(Frame::new(FrameType::HandshakeCookie, FrameFlags::NONE, payload))
    }
    ...
    pub fn process_complete(
        ...
        Ok(Frame::new(FrameType::HandshakeAck, FrameFlags::NONE, payload))
    }
}
...
impl ClientHandshake {
    ...
    pub fn init(&mut self) -> Result<Frame, FramingError> {
        ...
        let payload = bincode::serialize(&init)
            .map_err(|e| FramingError::Serialization(e.to_string()))?;
        
        self.state = HandshakeState::AwaitingCookie;
        
        Ok(Frame::new(FrameType::HandshakeInit, FrameFlags::NONE, payload))
    }
    ...
    pub fn process_cookie(&mut self, payload: &[u8]) -> Result<Frame, FramingError> {
        ...
        Ok(Frame::new(FrameType::HandshakeComplete, FrameFlags::NONE, payload))
    }
...
pub mod frames {
    ...
    pub fn ping(sequence: u64) -> Frame {
        Frame::new(FrameType::Ping, FrameFlags::NONE, sequence.to_be_bytes().to_vec())
    }
    
    /// Create a pong frame
    pub fn pong(sequence: u64) -> Frame {
        Frame::new(FrameType::Pong, FrameFlags::NONE, sequence.to_be_bytes().to_vec())
    }
    
    /// Create a close frame
    pub fn close(reason: &str) -> Frame {
        Frame::new(FrameType::Close, FrameFlags::NONE, reason.as_bytes().to_vec())
    }
    ...
    pub fn priority_data(payload: Vec<u8>) -> Frame {
        let flags = FrameFlags::new(
            FrameFlags::ENCRYPTED.as_byte() | FrameFlags::PRIORITY_HIGH.as_byte()
        );
        Frame::new(FrameType::Data, flags, payload)
    }
}
...
mod tests {
    ...
    fn test_full_handshake() {
        ...
        assert_eq!(complete_frame.header.frame_type, FrameType::HandshakeComplete);
        
        // Server processes completion
        let complete_payload: HandshakeComplete = bincode::deserialize(&complete_frame.payload).unwrap();
