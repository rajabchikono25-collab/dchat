...
pub struct UnifiedProofCommitment {
    ...
    /// Timestamp
    pub timestamp: u64,
    /// Committer's public key (Ed25519, 32 bytes)
    pub committer_id: Option<[u8; 32]>,
...
impl HardenedPoRW {
    ...
    pub fn submit_vote(&self, vote: IntegrationCommitteeVote) -> Result<(), IntegrationError> {
        ...
        // VRF proof minimum size: 32 bytes output + 64 bytes Ed25519 signature
        if vote.vrf_proof.len() < 96 {
            return Err(IntegrationError::VerificationFailed(
                "VRF proof too short (need output + signature)".to_string(),
            ));
        }

        // VRF proof structure:
        // - Bytes 0..32:  VRF output (H(signature || input))
        // - Bytes 32..96: Ed25519 signature over the VRF input
        //
        // To verify:
        // 1. Reconstruct VRF input: block_hash || height || subblock || voter_id
        // 2. Verify Ed25519 signature over input using voter's public key
        // 3. Compute expected_output = blake3(signature || input)
        // 4. Compare expected_output with claimed output
        // 5. Derive committee index from output
        let vrf_output: [u8; 32] = vote.vrf_proof[..32]
            .try_into()
            .map_err(|_| IntegrationError::VerificationFailed("Invalid VRF output".into()))?;
        let vrf_signature: [u8; 64] = vote.vrf_proof[32..96]
            .try_into()
            .map_err(|_| IntegrationError::VerificationFailed("Invalid VRF signature".into()))?;

        // Construct VRF input: deterministic from block context
        let mut vrf_input = Vec::with_capacity(32 + 8 + 1 + 32);
        vrf_input.extend_from_slice(vote.block_hash.as_bytes());
        vrf_input.extend_from_slice(&block_height.to_le_bytes());
        vrf_input.push(0u8); // subblock index (0 for block-level vote)
        vrf_input.extend_from_slice(&vote.voter_id);

        // Get voter's public key
        let public_key = VerifyingKey::from_bytes(&vote.voter_id)
            .map_err(|_| IntegrationError::VerificationFailed("Invalid voter public key".into()))?;

        // Verify Ed25519 signature over VRF input
        let ed_sig = ed25519_dalek::Signature::from_bytes(&vrf_signature);
        if let Err(e) = public_key.verify_strict(&vrf_input, &ed_sig) {
        ...
        // Compute expected VRF output: H(signature || input)
        let mut hasher = blake3::Hasher::new();
        hasher.update(&vrf_signature);
        hasher.update(&vrf_input);
        let expected_output = hasher.finalize();

        if *expected_output.as_bytes() != vrf_output {
            return Err(IntegrationError::VerificationFailed(
                "VRF output does not match recomputation".to_string(),
            ));
        }

        // Derive committee index from verified VRF output
        let vrf_output_val = u64::from_le_bytes(vrf_output[..8].try_into().unwrap());
        let expected_committee_index = (vrf_output_val % self.target_committee_size as u64) as u16;

        if vote.committee_index != expected_committee_index {
            return Err(IntegrationError::VerificationFailed(format!(
                "VRF committee index mismatch: claimed {} but computed {}",
                vote.committee_index, expected_committee_index
            )));
        }
    ...
    pub fn submit_proof_commitment(
        ...
        // Derive verifying key from commitment submitter's identity
        // The commitment signature must be verified against the submitter's public key
        // which is their relay identity in the PoRW system
        let committer = VerifyingKey::from_bytes(&commitment.committer_id.unwrap_or([0u8; 32]))
            .map_err(|_| {
                IntegrationError::CommitmentInvalid("Invalid committer public key".to_string())
            })?;
...
impl HardenedPoT {
    ...
    ///
    /// The committer_id is the Ed25519 public key of the relay submitting this
    /// transit proof commitment. It is verified against the signature.
    pub fn submit_transit_commitment(
        ...
        let commitment = UnifiedProofCommitment {
            ...
            committer_id: Some(committer_id),
        ...
        // Derive verifying key from committer's public key identity
        let committer = VerifyingKey::from_bytes(&committer_id).map_err(|_| {
            IntegrationError::CommitmentInvalid("Invalid committer public key".to_string())
        })?;
