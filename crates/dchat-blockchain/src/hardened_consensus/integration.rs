...
impl HardenedPoRW {
    ...
    pub fn submit_vote(&self, vote: IntegrationCommitteeVote) -> Result<(), IntegrationError> {
        ...
        // The VRF proof demonstrates the voter was selected for this committee
        // without revealing their selection probability until now.
        if vote.vrf_proof.len() < 64 {
            return Err(IntegrationError::VerificationFailed(
                "VRF proof too short".to_string(),
            ));
        }
        // VRF verification: check that vrf_proof was generated by voter_id's key
        // for this block_hash input. The proof structure contains:
        // - 32 bytes: VRF output (deterministic random value)
        // - 32+ bytes: proof of correct computation
        // Committee selection uses the VRF output modulo committee size
        let expected_committee_index = {
            let vrf_output_bytes: [u8; 32] = vote.vrf_proof[..32]
                .try_into()
                .map_err(|_| IntegrationError::VerificationFailed("Invalid VRF output".into()))?;
            let vrf_output_val = u64::from_le_bytes(vrf_output_bytes[..8].try_into().unwrap());
            (vrf_output_val % self.target_committee_size as u64) as u16
        };
        if vote.committee_index != expected_committee_index {
            return Err(IntegrationError::VerificationFailed(
                "VRF committee index mismatch".to_string(),
            ));
        }
