...
pub enum PartitionRequest<V> {
    /// Get a value
    Get { key: Vec<u8>, response: Sender<Option<V>> },
    /// Insert a value
    Insert { key: Vec<u8>, value: V, response: Sender<Result<Option<V>, ShardingError>> },
    /// Remove a value
    Remove { key: Vec<u8>, response: Sender<Option<V>> },
    /// Check if key exists
    Contains { key: Vec<u8>, response: Sender<bool> },
    /// Get partition size
...
impl<V: Clone + Send + Sync + 'static> ShardedState<V> {
    ...
    pub fn new(max_items_per_partition: usize) -> Self {
        Self::with_partitions(num_cpus::get(), max_items_per_partition)
    }
                ...
                .spawn(move || {
                    ...
                    while let Ok(request) = rx.recv() {
                        match request {
                            ...
                            PartitionRequest::Insert { key, value, response } => {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<V>, ShardingError> {
        ...
        self.partition_senders[partition]
            .send(PartitionRequest::Get { key: key.to_vec(), response: tx })
    ...
    pub fn insert(&self, key: Vec<u8>, value: V) -> Result<Option<V>, ShardingError> {
        ...
        self.partition_senders[partition]
            .send(PartitionRequest::Insert { key, value, response: tx })
            .map_err(|_| ShardingError::ChannelClosed)?;
    ...
    pub fn remove(&self, key: &[u8]) -> Result<Option<V>, ShardingError> {
        ...
        self.partition_senders[partition]
            .send(PartitionRequest::Remove { key: key.to_vec(), response: tx })
            .map_err(|_| ShardingError::ChannelClosed)?;
    ...
    pub fn contains(&self, key: &[u8]) -> Result<bool, ShardingError> {
        ...
        self.partition_senders[partition]
            .send(PartitionRequest::Contains { key: key.to_vec(), response: tx })
            .map_err(|_| ShardingError::ChannelClosed)?;
    ...
    pub fn iter_all(&self) -> Result<Vec<(Vec<u8>, V)>, ShardingError> {
        ...
        for partition in 0..self.partition_count {
            ...
            let entries = rx.recv_timeout(Duration::from_millis(CROSS_PARTITION_TIMEOUT_MS * 10))
                .map_err(|_| ShardingError::Timeout)?;
...
pub mod block_votes {
    use super::*;
    use crate::proof_of_relay_work::BlockVotes;
    ...
    impl ShardedBlockVotes {
        ...
        pub fn update_or_insert<F>(&self, block_hash: [u8; 32], default: BlockVotes, update_fn: F)
        where
            F: FnOnce(&mut BlockVotes),
        {
...
pub mod migration {
    ...
    where
        ...
        V: Clone,
    {
        let shards = LockFreeShardedState::new(num_cpus::get());
    ...
    pub fn benchmark_comparison<V: Clone + Default + Send + Sync + 'static>(
        ...
        let sharded: Arc<LockFreeShardedState<V>> = Arc::new(LockFreeShardedState::new(num_cpus::get()));
...
mod tests {
    ...
    fn test_consistent_hash_ring() {
        ...
        for count in &partition_counts {
            assert!(*count > 150 && *count < 350, "Uneven distribution: {:?}", partition_counts);
        }
    }
    ...
    fn test_sharded_state_with_workers() {
        ...
        // Insert
        state.insert(b"key1".to_vec(), "value1".to_string()).unwrap();
        state.insert(b"key2".to_vec(), "value2".to_string()).unwrap();
