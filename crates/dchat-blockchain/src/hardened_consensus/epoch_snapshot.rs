...
impl Region {
    pub fn from_country_code(code: &str) -> Self {
        match code.to_uppercase().as_str() {
            ...
            "GB" | "DE" | "FR" | "IT" | "ES" | "NL" | "SE" | "NO" | "FI" | "PL" | "CH" => {
                Region::Europe
            }
            "ZA" | "NG" | "KE" | "EG" | "MA" => Region::Africa,
            "CN" | "JP" | "KR" | "IN" | "SG" | "HK" | "TW" | "TH" | "VN" | "ID" | "PH" | "MY" => {
                Region::Asia
            }
...
impl StakerState {
    ...
    pub fn recalculate_power(&mut self) {
        ...
        self.effective_power = self.stake_amount.saturating_mul(self.temporal_multiplier) / 100;
    }
}
...
impl Snapshot {
    ...
    fn compute_merkle_root(&self) -> Hash {
        ...
        hasher.update(self.previous_root.as_bytes());
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, EpochError> {
        bincode::serialize(self).map_err(|e| EpochError::SerializationError(e.to_string()))
    }

    /// Deserialize from bytes
    pub fn from_bytes(data: &[u8]) -> Result<Self, EpochError> {
        bincode::deserialize(data).map_err(|e| EpochError::SerializationError(e.to_string()))
    }
}
...
impl SnapshotStore {
    ...
    pub fn latest(&self) -> Option<Arc<Snapshot>> {
        ...
        snapshots
            .keys()
            .max()
            .and_then(|k| snapshots.get(k).cloned())
    }
...
mod tests {
    ...
    fn test_snapshot_finalization() {
        ...
        for i in 0..MIN_ACTIVE_STAKERS + 5 {
            snapshot.upsert_staker(create_test_staker((i + 100) as u8, 1_000_000_000, 720));
        }
