...
impl DistributedObjectStorage {
    ...
    pub async fn new(config: ObjectStorageConfig) -> StorageResult<Self> {
        info!(
            "Initializing object storage: bucket={}, region={}",
        ...
        let bucket = Bucket::new(&config.bucket_name, region, credentials).map_err(|e| {
    ...
    pub async fn download_file(&self, object_key: &str, dest_path: &Path) -> StorageResult<()> {
    ...
    pub async fn delete(&self, object_key: &str) -> StorageResult<()> {
        ...
        self.bucket.delete_object(object_key).await.map_err(|e| {
    ...
    pub async fn get_metadata(&self, object_key: &str) -> StorageResult<ObjectMetadata> {
        ...
        let (head_object, _) = self.bucket.head_object(object_key).await.map_err(|e| {
            error!("Head object failed: {}", e);
        ...
        let etag = head_object.e_tag.unwrap_or_default();
