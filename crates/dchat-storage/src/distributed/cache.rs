...
impl DistributedCache {
    ...
    pub fn new(config: CacheConfig) -> StorageResult<Self> {
        info!("Connecting to Redis cluster with {} nodes", config.cluster_urls.len());
        
        let client = ClusterClient::new(config.cluster_urls.clone())
            .map_err(|e| {
    ...
    pub fn cache_message<T: Serialize>(
        &self,
        key: &str,
        message: &T,
    ) -> StorageResult<()> {
        self.cache_message_with_ttl(key, message, Duration::from_secs(self.config.default_ttl_seconds))
    }
    ...
    pub fn increment_counter(
        ...
        let new_value: i64 = conn.incr(key, delta)
            .map_err(|e| {
