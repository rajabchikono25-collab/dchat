...
impl SearchManager {
    ...
    pub fn search(&self, query: &str, filters: SearchFilters) -> Result<Vec<SearchResult>> {
        ...
        let search_users = filters.search_type.is_none()
            || matches!(
                filters.search_type,
                Some(SearchType::All) | Some(SearchType::Users)
            );

        let search_bots = filters.search_type.is_none()
            || matches!(
                filters.search_type,
                Some(SearchType::All) | Some(SearchType::Bots)
            );
    ...
    fn matches_bot_query(
        ...
        if !filters.tags.is_empty() {
            let has_matching_tag = filters.tags.iter().any(|tag| metadata.tags.contains(tag));
            if !has_matching_tag {
                return false;
            }
        }
