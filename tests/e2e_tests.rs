...
async fn test_end_to_end_encrypted_message() {
    ...
    let mut bob_handshake =
        NoiseHandshake::respond(NoisePattern::XX, bob_keypair.private_key()).unwrap();
...
fn test_hierarchical_key_derivation() {
    ...
    assert_ne!(
        device1_key.public_key().as_bytes(),
        device2_key.public_key().as_bytes()
    );
    ...
    assert_ne!(
        conv1_key.public_key().as_bytes(),
        conv2_key.public_key().as_bytes()
    );
}
...
async fn test_event_bus_pub_sub() {
    ...
    event_bus
        .publish(Event::MessageReceived { message })
        .await
        .unwrap();
...
async fn test_simple_message_send() {
    ...
    let in_order = order.register_message(sender_id.to_string(), SequenceNumber(0), message.id);
    ...
    assert_eq!(
        tracker.get_status(&message.id),
        Some(DeliveryStatus::OnChain)
    );
...
async fn test_storage_and_persistence() {
    ...
    println!(
        "✅ User retrieved: {} ({})",
        retrieved_user.username, retrieved_user.id
    );
    ...
    assert_eq!(
        msg1_hash, msg3_hash,
        "Identical content should have identical hash"
    );
    assert_ne!(
        msg1_hash, msg2_hash,
    ...
    assert_eq!(
        dedup_store.ref_count(&msg1_hash),
        2,
        "Message 1 should have 2 references"
    );
    assert_eq!(
        dedup_store.ref_count(&msg2_hash),
        1,
        "Message 2 should have 1 reference"
    );
